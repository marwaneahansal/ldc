...
export class ClientService {
  ...
  reserveCar(reservationData: {
    ...
    return this.http.post(
      'http://localhost:8080/api/reservations/creer',
      reservationData
    );
  }
  ...
  confirmReservation(id: number, signatureData: string): Observable<any> {
    ...
    return this.http.post(`${this.apiUrl2}/${id}`, null).pipe(
      switchMap((response: any) => {
        ...
        return this.http
          .post(
            `${this.apiUrl2}/${idContrat}/generate-pdf-with-signature`,
            formData,
            {
  ...
  getNombreReservations(id: number | null = null): Observable<number> {
    return this.http.get<number>(
      `${this.apiUrl1}/countReservationClient/${id}`
    );
  }
  getNombreReservationsStatu(
    id: number | null = null,
    statu: String
  ): Observable<number> {
    return this.http.get<number>(
      `${this.apiUrl1}/countReservationClientStatu/${id}/${statu}`
    );
  }
}
